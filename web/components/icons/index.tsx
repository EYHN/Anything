export { default as Storage } from './storage.svg';
export { default as Switch } from './switch.svg';
export { default as Archive } from './archive.svg';
export { default as Time } from './time.svg';
export { default as Tag } from './tag.svg';
export { default as Trash } from './trash.svg';
export { default as Folder } from './folder.svg';
export { default as RightBar } from './right-bar.svg';
export { default as Edit } from './edit.svg';
export { default as Edit2 } from './edit-2.svg';
export { default as Size } from './size.svg';
export { default as Bit } from './bit.svg';
export { default as ColorSpace } from './color-space.svg';
export { default as Empty } from './empty.svg';
export { default as Rotate } from './rotate.svg';
export { default as Search } from './search.svg';
export { default as Sort } from './sort.svg';
export { default as Downward } from './downward.svg';
export { default as Filter } from './filter.svg';
export { default as Resize } from './resize.svg';
export { default as List } from './list.svg';
export { default as Width } from './width.svg';
export { default as Height } from './height.svg';
export { default as PlusCircle } from './plus-circle.svg';
export { default as Close } from './close.svg';
export { default as NewlyBuild } from './newlybuild.svg';
export { default as Record } from './record.svg';
export { default as People } from './people.svg';
export { default as RecordDisc } from './record-disc.svg';
export { default as Entertainment } from './entertainment.svg';
export { default as MusicRhythm } from './music-rhythm.svg';
export { default as MusicList } from './music-list.svg';
export { default as Music } from './music.svg';
export { default as Piano } from './piano.svg';
