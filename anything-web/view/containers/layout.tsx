  ...
  return <div className={classes.container} ref={containerRef}>
    {
      ...
      <SplitView className={classes.splitView} minSize={[200, 440]} size={size} width={containerWidth} height={containerHeight} onSizeChange={handleSplitViewSizeChange} grow={[0,1]}>
        {[
          (width, height) => <div className={classes.left} key="left">
            {left && left(width, height)}
          </div>,
          (width, height) => <div className={classnames(classes.maincontainer)} key="maincontainer">
            <div className={classes.top}>
              {top && top(width, 48)}
            </div>
            <SplitView minSize={[220, 220]} size={innerSize} width={width} height={height - 48 - 28} onSizeChange={handleInnerSplitViewSizeChange} grow={[1,0]} >
              {[
                (width, height) => <div className={classes.center}>
                  {center && center(width, height)}
                </div>,
                (width, height) => <div className={classes.right}>
