...
const File: React.FunctionComponent<FileProps> = ({entry, width, height, focus, style, onDoubleClick, onMouseDown, imgRef, textRef}) => {
  ...
  const imageSize = Math.min(
    height - containerPadding - textHeight,
  ...
  return <div style={{ position: 'absolute', ...style }}>
    <FileIcon file={entry} width={imageSize} height={imageSize} className={classes.image}  onDoubleClick={onDoubleClick} onMouseDown={onMouseDown} ref={imgRef} />
    <div className={classes.textStyle}><span onDoubleClick={onDoubleClick} onMouseDown={onMouseDown} className={classes.spanStyle} ref={textRef}>{entry.name}</span></div>
  </div>
}
