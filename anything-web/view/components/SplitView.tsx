...
const SplitView: React.FunctionComponent<ISplitViewProps> = ({ className, size, onSizeChange, children, width, height, grow, minSize }) => {
  ...
  return (
    <div className={classNames(classes.splitView, className)} style={{ width, height }}>
      ...
      <div className={classes.sashContinter}>{sashs}</div>
      <div className={classes.viewContinter}>{views}</div>
    </div>
  );
};
