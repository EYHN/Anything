...
export const BoxSelectContainer = React.forwardRef<HTMLDivElement, BoxSelectContainerProps>(({children, style, onMouseMove, onScroll, onSelectStart, onSelectRect, onSelectEnd, ...otherProps}, ref) => {
  ...
  const handleOnScroll = React.useCallback((e: React.UIEvent<HTMLDivElement>) => {
    if (typeof onScroll === 'function') onScroll(e);
    scrollTopRef.current = e.currentTarget.scrollTop;
  }, [scrollTopRef])

  const handleOnMouseDown = React.useCallback((e: React.MouseEvent<HTMLDivElement>) => {
  ...
  const handleOnMouseMove = React.useCallback((e: MouseEvent) => {
    function update() {
  ...
  return <>
    ...
    <div style={{position: 'fixed', width: clientRect?.width, height: clientRect?.height, top: clientRect?.top, left: clientRect?.left, overflow: 'hidden', pointerEvents: 'none'}}>
      {
...
const useBoxSelectContainer = (addprops: {onSelectRect?: BoxSelectContainerProps["onSelectRect"], onSelectStart?: BoxSelectContainerProps["onSelectStart"], onSelectEnd?: BoxSelectContainerProps["onSelectEnd"]}) => {
  const component = React.useMemo(() => React.forwardRef<HTMLDivElement, BoxSelectContainerProps>((props, ref) => {
