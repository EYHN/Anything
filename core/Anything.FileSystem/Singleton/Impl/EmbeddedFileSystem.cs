...
public class EmbeddedFileSystem : BaseStaticFileSystem
{
    ...
    protected override ValueTask<IEnumerable<(string Name, FileStats Stats)>> ReadDirectory(string path)
    {
        throw new NotSupportedException("The embedded file system does not support read directory operation.");
    }

    protected override async ValueTask<ReadOnlyMemory<byte>> ReadFile(string path)
    {
    ...
    protected override ValueTask<FileStats> Stat(string path)
    {
        var fileInfo = _embeddedFileProvider.GetFileInfo(path);
        if (!fileInfo.Exists)
        {
    ...
    protected override async ValueTask<T> ReadFileStream<T>(string path, Func<Stream, ValueTask<T>> reader)
    {
        var fileInfo = _embeddedFileProvider.GetFileInfo(path);
        if (!fileInfo.Exists)
        {
